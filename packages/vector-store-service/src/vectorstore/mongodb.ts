...
export async function apply(
    ...
    const MongoDB = await importMongoDB()
    const MongoDBAtlas = await importMongoDBAtlasVectorSearch()
    ...
    plugin.registerVectorStore('mongodb', async (params) => {
        ...
        const vectorStore = new MongoDBAtlas.MongoDBAtlasVectorSearch(
            embeddings,
            {
                collection,
...
async function importMongoDBAtlasVectorSearch() {
    try {
        return await import('@langchain/mongodb')
    } catch (err) {
        logger.error(err)
        throw new Error(
            'Please install mongodb-atlas-vector-search as a dependency with, e.g. `npm install -S mongodb-atlas-vector-search`'
        )
    }
}
