    ...
    implements EmbeddingsRequester
{
    ...
    async completion(params: ModelRequestParams): Promise<ChatGeneration> {
        try {
            ...
            const messagePart = partAsType<ChatMessagePart>(part.parts[0])
            const chatFunctionCallingPart = partAsType<ChatFunctionCallingPart>(
                part.parts[0]
            )
            ...
            for (const source of parsedResponse.candidates[0].groundingMetadata
                ?.groundingChunks ?? []) {
